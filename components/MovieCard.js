...
const MovieCard = ({ movie }) => {
  return (
    <Link href={`/movie/${movie.id}`} className="bg-white rounded-md">
        <div className="bg-white shadow-sm rounded-md cursor-pointer">
            <Image src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={`${movie.title}`} width={700} height={800} className="rounded-t-md" />
